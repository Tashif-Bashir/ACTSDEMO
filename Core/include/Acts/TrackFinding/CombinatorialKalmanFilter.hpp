...
struct CombinatorialKalmanFilterOptions {
  ...
  ///       which new tips are to be created.
  ///
  ...
  /// @param trackStateCandidates list to be filled with the new states
  /// @param isOutlier flag to be set to true of the states are outliers
  /// @param logger a logger for messages
  using TrackStateCandidateCreator = Delegate<Result<std::pair<
      typename std::vector<typename traj_t::TrackStateProxy>::iterator,
      typename std::vector<typename traj_t::TrackStateProxy>::iterator>>(
      ...
      std::size_t prevTip, traj_t& trajectory,
   ...
   public:
    ...
    Result<void> filter(const Surface* surface, propagator_state_t& state,
      ...
      if (slBegin != slEnd) {
        ...
        TrackStateRangeResult selectorResult = trackStateCandidateCreator(
            state.geoContext, calibrationContext, *surface, boundState, slBegin,
            slEnd, prevTip, *result.stateBuffer, result.trackStateCandidates,
            isOutlier, logger());
